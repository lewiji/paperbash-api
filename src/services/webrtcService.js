...
export function startWebRTCServer() {
  ...
  console.log(chalk`{green info}: {magenta WS server started on} :{bgBlack {green.bold ${PORT}}}`);
...
export function registerLobby(joinCode) {
  console.log(`Registering lobby code ${joinCode}`);
  joinCodes.set(joinCode);
}
...
function joinLobby(peer, pLobby) {
  const lobbyName = pLobby;

  if (joinCodes.has(lobbyName)) {
...
function parseMsg(peer, msg) {
  const command = msg.indexOf(':');

  msg = msg.slice(command - 1);
